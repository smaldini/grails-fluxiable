      ...
      if (Fluxiable.class.isAssignableFrom(domainClass.clazz)) {
        domainClass.clazz.metaClass {
          'static' {
            getRecentActivities {->
              def clazz = delegate
              ActivityLink.getRecentActivities(clazz).list()
            }
          }
          ...
          addActivity { poster, String code, String cachedLabel, String cachedDesc = null ->
            ...
            c.save()
            def link = new ActivityLink(activity: c, activityRef: delegate.id, type: GrailsNameUtils.getPropertyName(delegate.class))
            link.save()
            try {
          ...
          getActivities = {->
            ActivityLink.getActivities(delegate).list()
          }

          getTotalActivities = {->
            ActivityLink.getTotalActivities(delegate).get()
          }
